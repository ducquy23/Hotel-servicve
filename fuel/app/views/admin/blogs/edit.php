<div class="content-header row">
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-left mb-0">Chỉnh sửa Blog</h2>
...
<div class="card">
    <div class="card-body">
        ...
        <form method="POST" action="<?= \Uri::create('admin/blogs/edit/' . $blog['id']) ?>" enctype="multipart/form-data">
            <div class="row">
                <div class="col-md-8">
                    <div class="mb-3">
                        <label class="form-label" for="title">Tiêu đề <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="title" name="title" 
                               value="<?= htmlspecialchars(\Input::post('title', $blog['title'])) ?>" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="content" id="content" style="display: none;" required><?= \Input::post('content', $blog['content']) ?></textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="excerpt" name="excerpt" rows="3" 
                                  placeholder="Mô tả ngắn gọn về bài viết..."><?= htmlspecialchars(\Input::post('excerpt', $blog['excerpt'])) ?></textarea>
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" required>
                                    <option value="draft" <?= \Input::post('status', $blog['status']) === 'draft' ? 'selected' : '' ?>>Bản nháp</option>
                                    <option value="published" <?= \Input::post('status', $blog['status']) === 'published' ? 'selected' : '' ?>>Xuất bản</option>
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="category" name="category" required>
                                    ...
                                    <?php foreach ($categories as $value => $label): ?>
                                        <option value="<?= $value ?>" <?= \Input::post('category', $blog['category']) === $value ? 'selected' : '' ?>><?= $label ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" class="form-control" id="featured_image" name="featured_image" 
                                       accept="image/*">
                                <small class="text-muted">Chọn ảnh đại diện mới (để trống nếu không thay đổi)</small>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="position-relative d-inline-block">
                                    <img src="<?= \Uri::base() . htmlspecialchars($blog['featured_image']) ?>" 
                                         alt="Ảnh đại diện" style="height: 100px; border-radius: 6px; border: 1px solid #eee;">
                                    <button type="button" class="btn btn-sm btn-danger js-delete-blog-image" 
                            ...
                            <?php if ($blog['published_at']): ?>
                                <div class="mb-3">
                                    ...
                                    <p class="form-control-plaintext"><?= date('d/m/Y H:i', strtotime($blog['published_at'])) ?></p>
                                </div>
                            <?php endif; ?>
                            
                            <div class="mb-3">
                                <label class="form-label">Ngày tạo</label>
                                <p class="form-control-plaintext"><?= date('d/m/Y H:i', strtotime($blog['created_at'])) ?></p>
                            </div>
                            
                            <div class="mb-3">
                                <label class="form-label">Cập nhật lần cuối</label>
                                <p class="form-control-plaintext"><?= date('d/m/Y H:i', strtotime($blog['updated_at'])) ?></p>
                            </div>
